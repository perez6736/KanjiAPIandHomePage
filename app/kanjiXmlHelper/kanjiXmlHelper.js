...
function formatCharacterObject(KanjiObject) {
  ...
  let heisig = getHeisigNumber(KanjiObject);
...
function getAllReadings(KanjiObject) {
  ...
  if (!Array.isArray(readings)) {
    // if there is one value just return it.
    return readings;
  }
  let formattedObject = {};
...
function getEnglishMeaningsFromKanjiObject(KanjiObject) {
  ...
  let englishMeanings = meanings.filter(
...
function getNonEnglishMeaningsFromKanjiObject(KanjiObject) {
  ...
  let nonEnglishMeanings = meanings.filter(
    (meaning) => typeof meaning != "string"
  ); //filters out non English
  let formattedObject = {};
...
// this takes a single kanjiObject that has been parsed from xml2js
// returns the index within the heisig list of kanji
// can return null or a string of a number.
function getHeisigNumber(KanjiObject) {
  let ref = KanjiObject.dic_number.dic_ref;
  for (i = 0; i < ref.length; i++) {
    if (ref[i].dr_type === "heisig") {
      return ref[i].text;
    }
  }
  return null;
...
const kanjiXml = {
  ...
  getKanjisInfo: (kanjiCharacters) => {
    return new Promise((resolve) => {
      XMLtoJSON.then((results) => {
        let kanjis = [];
        kanjiCharacters.forEach((kanji) => {
          // for each kanji passed queried by client.
          for (let i = 0; i < results.character.length; i++) {
            // loop through entire kanji list
            if (results.character[i].literal[0] === kanji) {
              // if there is a match push into kanjis array
              const formattedKanjiJSON = formatCharacterObject(
                results.character[i]
              );
  ...
  getKanjiByGrade(gradeLevel) {
    return new Promise((resolve) => {
      XMLtoJSON.then((results) => {
        ...
        for (let i = 0; i < results.character.length; i++) {
          if (
            results.character[i].misc.grade != undefined &&
            parseInt(results.character[i].misc.grade) === gradeLevel
          ) {
            const formattedKanjiJSON = formatCharacterObject(
              results.character[i]
            );
  ...
  getKanjiByJLPTn(jlptLevel) {
    return new Promise((resolve) => {
      XMLtoJSON.then((results) => {
        ...
        for (let i = 0; i < results.character.length; i++) {
          if (
            results.character[i].misc.jlpt != undefined &&
            parseInt(results.character[i].misc.jlpt) === jlptLevel
          ) {
            const formattedKanjiJSON = formatCharacterObject(
              results.character[i]
            );
    ...
    return new Promise((resolve) => {
      XMLtoJSON.then((results) => {
        ...
        for (let i = 0; i < results.character.length; i++) {
          if (
            results.character[i].misc.grade != undefined &&
            parseInt(results.character[i].misc.grade) < 9
          ) {
            const formattedKanjiJSON = formatCharacterObject(
              results.character[i]
            );
  ...
  getKanjiFromMeaning(meaning) {
    return new Promise((resolve) => {
      XMLtoJSON.then((results) => {
        ...
        for (let i = 0; i < results.character.length; i++) {
          if (
            results.character[i].reading_meaning != undefined &&
            results.character[i].reading_meaning.rmgroup.meaning != undefined
          ) {
            if (
              typeof results.character[i].reading_meaning.rmgroup.meaning ===
              "string"
            ) {
              // if its a string that means only one meaning.
              if (
                results.character[i].reading_meaning.rmgroup.meaning === meaning
              ) {
                console.log(
                  results.character[i].reading_meaning.rmgroup.meaning
                );
                const formattedKanjiJSON = formatCharacterObject(
                  results.character[i]
                );
              ...
              // if its an array of meanings we loop through it.
              for (
                let j = 0;
                j < results.character[i].reading_meaning.rmgroup.meaning.length;
                j++
              ) {
                if (
                  results.character[i].reading_meaning.rmgroup.meaning[j] ===
                  meaning
                ) {
                  const formattedKanjiJSON = formatCharacterObject(
                    results.character[i]
